...
Var_Dict = [
        ...
        #{'variable':'lead_jet_pt',                  'nBins':50,             'xLow':0,               'xHi':1000,     'title':'nominal Jet p_{T} [GeV]'},
        {'variable':'met_pt',                  'nBins':30,             'xLow':0,               'xHi':600,	'title':'p_{T}^{miss} [GeV]'},
        #{'variable':'MET_pt_nom',                  'nBins':100,             'xLow':0,               'xHi':1000,      'title':'nominal p_{T}^{miss} [GeV]'}
	]

_nVars = len(Var_Dict)

#_cuts = "((lep_category==6 || lep_category==7) && Z_pt>60 && Z_mass > 61 && Z_mass < 121 && ngood_jets < 2 && abs(emulatedMET_phi-Z_phi) > 2.8 && emulatedMET > 100)"
...
_cuts = "((lep_category==1 || lep_category==3) && ngood_jets < 2)"# && Z_mass > 76 && Z_mass < 106 && Z_pt>60 && ngood_jets < 2 && ngood_bjets == 0)"# && met_pt > 40 && Flag_METFilters == 1 && Flag_CSCTightHaloFilter == 1)"# && abs(delta_met_rec) < 1)"
...
#   #leg.AddEntry(DY,"DY","F")
   ...
   mchists = [None] * _nPDs
   mc = TH1F('data2', 'data2',_nBins, _xLow, _xHi)
   mc2 = TH1F('data2', 'data2',_nBins, _xLow, _xHi)
   for j in range(_nPDs):
      pd = _PDs[j]
      chain = TChain("Events")
      for file in _dict_yml_ctgry[pd]['files']:
      ...
      chain.SetProof()
      hist_name = pd.split('_')[0]
      mchists[j] = TH1F('data2', 'data2',_nBins, _xLow, _xHi)
      chain.Project('data2', 'MET_pt_nom', _cuts)
      #chain.Project('data', 'Z_mass', "(lep_category==2  && Z_pt>60 && met_pt>40 && ngood_jets<2 && ngood_bjets==0)")
      #dathists[j].Scale(5.0)
      mchists[j].SetName(hist_name)
      chain.Reset()
      chain.Delete()
      mc.Add(mchists[j])
   ...
   leg  = TLegend(.7,.7,.9,.9, "", "fNDC")
#   #leg.AddEntry(Other, "Other","F")
   leg.AddEntry(mc,"New JECs","P")
   leg.AddEntry(dat, "Old JECs","P")
#   #leg.AddEntry(WW,"WW","F")
#   #leg.AddEntry(TT,"TT","F")
#   #leg.AddEntry(VVV,"VVV","F")
#   #leg.AddEntry(DY,"DY","F")
   ...
   mc.SetMinimum(1.0)
   mc.SetMaximum(1000000000)
   mc.SetTitle(" ;"+ title + ";Events / Bin")
